...
import { ArrowLeft, Car, ChevronRight, FileKey2, Gauge, Mail, ShieldCheck, Wrench } from "lucide-react";
...
export default async function TuneWorkspace({params}){
  ...
  return <main className={styles.page}>
    <header className={styles.topbar}><Link href="/" className={styles.back}><ArrowLeft size={15}/>Dashboard</Link><div className={styles.brand}><img src="/subpar-logo.png" alt="Subpar Tuning"/><div><b>SUBPAR OS</b><span>LIVE TUNE WORKSPACE</span></div></div><div className={styles.links}><Link href="/messages">Messages <ChevronRight size={13}/></Link><Link href={`/files/${project.projectNumber}`}>Files <ChevronRight size={13}/></Link></div></header>

    <section className={styles.hero}><div><span>{project.projectNumber} · {String(project.platform||"TUNE").toUpperCase()}</span><h1>{customer.name||customer.email||"Customer"} — {vehicleName(vehicle)}</h1><p>{[vehicle.chassis,vehicle.engine,project.fuelTarget,`Rev ${project.currentRevision||0}`].filter(Boolean).join(" · ")}</p><div className={styles.badges}><b>{nice(project.status)}</b><b>{nice(project.waitingOn)} ownership</b><b>{nice(project.priority)} priority</b></div></div><aside><span>NEXT ACTION</span><h2>{project.nextAction||"Review project status"}</h2><p>All live actions stay attached to this project’s immutable history.</p></aside></section>
