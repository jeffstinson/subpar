...
import { Archive, ArrowLeft, Bot, Car, ChevronRight, FileKey2, Gauge, Mail, PackageCheck, ShieldCheck, Wrench } from "lucide-react";
...
export default async function TuneWorkspace({params}){
  ...
  return <main className={styles.page}>
    <header className={styles.topbar}><Link href="/" className={styles.back}><ArrowLeft size={15}/>Dashboard</Link><div className={styles.brand}><img src="/subpar-logo.png" alt="Subpar Tuning"/><div><b>SUBPAR OS</b><span>LIVE TUNE WORKSPACE</span></div></div><div className={styles.links}><Link href={`/delivery/${project.projectNumber}`}>Revision Delivery <ChevronRight size={13}/></Link><Link href={`/closeout/${project.projectNumber}`}>Tune Lifecycle <ChevronRight size={13}/></Link><Link href="/intelligence">Vehicle Intelligence <ChevronRight size={13}/></Link><Link href="/messages">Messages <ChevronRight size={13}/></Link><Link href={`/files/${project.projectNumber}`}>Files <ChevronRight size={13}/></Link></div></header>
    ...
    <div className={styles.layout}>
      ...
      <aside className={styles.stack}>
        ...
        <section className={styles.panel}><header><div><span>QUICK ACCESS</span><h2>Keep work in context</h2></div><Gauge size={18}/></header><div className={styles.actions}><Link href={`/delivery/${project.projectNumber}`}><PackageCheck size={14}/>Revision QA & delivery<ChevronRight size={12}/></Link><Link href={`/closeout/${project.projectNumber}`}><Archive size={14}/>Closeout & tune lifecycle<ChevronRight size={12}/></Link><Link href="/reviews"><Gauge size={14}/>Datalog Review Cockpit<ChevronRight size={12}/></Link><Link href="/messages"><Mail size={14}/>Customer communications<ChevronRight size={12}/></Link><Link href={`/files/${project.projectNumber}`}><FileKey2 size={14}/>Project files<ChevronRight size={12}/></Link>{project.projectNumber==="SP-1842"&&<Link href={`/log-review/${project.projectNumber}`}><Gauge size={14}/>Legacy deep log demo<ChevronRight size={12}/></Link>}</div></section>
