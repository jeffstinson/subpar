...
import { ArrowLeft, Bot, Car, ChevronRight, FileKey2, Gauge, Mail, PackageCheck, ShieldCheck, Wrench } from "lucide-react";
...
export default async function TuneWorkspace({params}){
  ...
  return <main className={styles.page}>
    <header className={styles.topbar}><Link href="/" className={styles.back}><ArrowLeft size={15}/>Dashboard</Link><div className={styles.brand}><img src="/subpar-logo.png" alt="Subpar Tuning"/><div><b>SUBPAR OS</b><span>LIVE TUNE WORKSPACE</span></div></div><div className={styles.links}><Link href={`/delivery/${project.projectNumber}`}>Revision Delivery <ChevronRight size={13}/></Link><Link href="/intelligence">Vehicle Intelligence <ChevronRight size={13}/></Link><Link href="/messages">Messages <ChevronRight size={13}/></Link><Link href={`/files/${project.projectNumber}`}>Files <ChevronRight size={13}/></Link></div></header>
