...
export async function POST(request,context){
  try{
    ...
    let data;
    if(action==="save")data=await saveCloseoutDraft({projectNumber:project,customerSummary:body.customerSummary||"",aftercareNotes:body.aftercareNotes||"",followupDays:body.followupDays??7,actor});
    else if(action==="qa")data=await runCloseoutQa({projectNumber:project,actor});
    else if(action==="approve")data=await approveCloseout({projectNumber:project,actor});
