...
import { AlertTriangle, Bot, CheckCircle2, ClipboardList, FileText, Gauge, RefreshCw, Settings2, ShieldCheck, Wrench } from "lucide-react";
...
export default function IntelligenceClient({vehicles,source="demo",validation=null}){
  ...
  const [engine,setEngine]=useState(selected?.engineFamilies?.includes("B58TU")?"B58TU":selected?.engineFamilies?.[0]||"B58TU");
  const [platform,setPlatform]=useState("MHD");
  const [fuel,setFuel]=useState("E40");
  const localResolution=useMemo(()=>resolveVehicleIntelligence({make:selected?.make,model:selected?.model,chassis:selected?.chassis?.[0],engine,platform,fuel}),[selected,engine,platform,fuel]);
  ...
  useEffect(()=>{
    let canceled=false;
    const payload={make:selected?.make,model:selected?.model,chassis:selected?.chassis?.[0],engine,platform,fuel};
    setResolution(localResolution);
    setResolving(true);
    fetch("/api/v1/intelligence",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({vehicle:payload,principal:"doug"})})
      .then(async response=>{const body=await response.json();if(!response.ok)throw new Error(body.error||"Unable to resolve intelligence");return body.resolution})
      .then(next=>{if(!canceled&&next)setResolution(next)})
      .catch(()=>{})
      .finally(()=>{if(!canceled)setResolving(false)});
    return()=>{canceled=true};
  },[selected,engine,platform,fuel,localResolution]);

  const ready=resolution.state==="workflow_ready";

  return <section className={styles.workspace}>
    <aside className={styles.selector}>
      <div className={styles.sectionHead}><div><span>TEST A CONFIGURATION</span><h2>Vehicle context</h2></div>{resolving?<RefreshCw size={18}/>:<Settings2 size={18}/>}</div>
      ...
      <div className={styles.safety}><ShieldCheck size={15}/><p>{resolution.safetyNote||"Doug remains the final compatibility approval gate."}</p></div>
    </aside>
    ...
    <div className={styles.main}>
      ...
      <section className={styles.grid2}>
        ...
        <article className={styles.panel}>
          ...
          <div className={styles.autoList}>{(resolution.automations||[]).map((item,index)=><div key={`${item}-${index}`}><i>{index+1}</i><span>{String(item).replaceAll("_"," ")}</span></div>)}</div>
        </article>
      </section>
