...
import { intelligenceSummary, vehicleIntelligenceCatalog } from "../lib/vehicle-intelligence";
...
export default function IntelligencePage(){
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.hero}>
      <div><span className={styles.eyebrow}>CHASSIS → ENGINE → PLATFORM → WORKFLOW</span><h1>The car should configure the project.</h1><p>Subpar OS resolves the vehicle and tuning platform into the exact prerequisites, logging recipe, parameter-pack placeholder and automation path Doug expects. ROM/DME eligibility remains an explicit tuner approval—not an assumption.</p><div className={styles.heroMeta}><span><Database size={13}/>{summary.vehicles} curated chassis presets</span><span><Cpu size={13}/>{summary.engines.length} engine families</span><span><Bot size={13}/>{summary.platforms.length} tuning platforms</span></div></div>
    ...
    <IntelligenceClient vehicles={vehicleIntelligenceCatalog}/>
  </main>;
}
