...
export default function OutboundClient({project,initialQueue=[],sendEnabled=false}){
  ...
  async function createDraft(){
    ...
    try{
      ...
      if(data.dryRun){
        setQueue(current=>[{...data,id:data.id||id(),created_at:new Date().toISOString(),recipient,to:recipient,payload:{body},created_by:"Doug Talmadge"},...current]);
  ...
  async function action(item,type){
    ...
    try{
      ...
      setStatus(type==="approve"?"Draft approved for provider send.":type==="send"?"Gmail send completed and provider ID was recorded.":"Draft canceled.");
    }catch(error){setStatus(error.message)}finally{setBusy("")}
  }
  ...
  return <div className={styles.layout}>
    ...
    <section className={styles.queue}>
      ...
      <div className={styles.items}>{queue.map(item=><article key={item.id}>
        ...
        <h3>{item.subject}</h3><p>{item.payload?.body||item.body||""}</p>
        <div className={styles.meta}><span>To {item.recipient||item.to}</span><span>{item.created_by||"Subpar OS"}</span>{item.provider_message_id&&<span>Gmail ID {item.provider_message_id}</span>}</div>
      ...
      {!queue.length&&<div className={styles.empty}><Clock3 size={22}/><b>No outbound messages queued</b><p>Drafts show up here before they are allowed anywhere near the Gmail send API.</p></div>}
    </section>
  </div>;
}
