...
export default async function OutboundPage(){
  ...
  const queue=await listOutboundActions({limit:50});
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.rules}><span><ShieldCheck size={14}/>Staff may prepare drafts</span><span><ShieldCheck size={14}/>Owner/tuner approval required</span><span><ShieldCheck size={14}/>Intake links are generated only at send-time</span><span><ShieldCheck size={14}/>Provider acceptance is recorded before secondary bookkeeping</span></section>
