...
export default function SecurityPage() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.hero}>
      <div><span className={styles.eyebrow}>PHASE 3 • IDENTITY + PRIVATE FILES</span><h1>Doug and the customer no longer share the same trust boundary.</h1><p>Subpar OS now has explicit internal roles, a separate customer portal identity, default-deny permissions, private file buckets and a server broker for short-lived upload/download tickets. Production authentication remains gated until the dedicated Supabase project is connected.</p></div>
      ...
      <div className={styles.main}>
        ...
        <section className={styles.panel}>
          ...
          <div className={styles.flow}>
            ...
            <div><span>2</span><div><b>Server creates a short-lived ticket</b><small>Downloads expire quickly. Uploads receive a unique, non-overwriting path rather than replacing an old tune file.</small></div></div>
            <div><span>3</span><div><b>File stays in a private bucket</b><small>Stock files, revisions, datalogs, parameter packs and customer attachments are separated by purpose.</small></div></div>
            <div><span>4</span><div><b>File metadata remains tied to the project/revision/log</b><small>That preserves immutable tune history while keeping customer-facing visibility separate from internal files.</small></div></div>
          </div>
        </section>
      </div>
      ...
      <aside className={styles.side}>
        ...
        <section className={styles.panel}>
          ...
          <div className={styles.apiLinks}>
            ...
            <a href="/api/v1/readiness">Persistence readiness <ChevronRight size={13}/></a>
          </div>
        </section>
      </aside>
    </section>

    <section className={styles.next}><div className={styles.nextInner}><div><span className={styles.eyebrow}>NEXT STEP AFTER SUPABASE IS ATTACHED</span><h2>Turn preview principals into actual Doug/customer sessions.</h2><p>Run the identity/storage migration, create Doug as the first owner, validate Alex-style portal access against synthetic seeded data, then enforce auth on internal and customer routes. Wix and Gmail still wait until those permission boundaries are proven.</p></div><div className={styles.steps}>{["Run 0002_identity_storage.sql","Create Doug owner membership","Test customer portal identity","Test signed file tickets","Then enforce protected routes"].map((step,index)=><div key={step}><span>{index+1}</span><b>{step}</b></div>)}</div></div></section>
  </main>;
}
