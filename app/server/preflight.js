...
const expectedTables = [
  ...
  "import_batches","import_items","intake_requests","integration_cutovers","schema_migrations","connection_tests","setup_checkpoints",
];
...
export async function probeSupabaseSchema(){
  ...
  if(!missing.includes("schema_migrations")){
    ...
    if(!error)versions=data||[];
  }
  return {
    provider:"supabase",
    status:missing.length?"fail":"pass",
    detail:missing.length?`${missing.length} expected table(s) missing`:`${expectedTables.length} expected tables available`,
    latencyMs:ms(start),
    tables:results,
    missing,
    migrations:versions,
...
export async function probePrivateStorage(){
  ...
  return {
    provider:"storage",
    status:missing.length||publicBuckets.length?"fail":"pass",
    detail:missing.length?`${missing.length} private bucket(s) missing`:publicBuckets.length?`${publicBuckets.length} Subpar bucket(s) unexpectedly public`:`${expected.length} private buckets verified`,
    latencyMs:ms(start),
    buckets:actual.filter(bucket=>expected.includes(bucket.id)||expected.includes(bucket.name)),
    missing,
    publicBuckets,
  };
}
...
async function storeConnectionTest(result,testedBy){
  ...
  try{
    ...
    await supabase.from("connection_tests").insert({
      ...
      test_type:result.provider==="supabase"?"schema":result.provider==="storage"?"private_buckets":"oauth_connection",
      status:result.status,
      latency_ms:result.latencyMs??null,
      detail:result.detail||null,
      metadata:{
        missing:result.missing||undefined,
        migrations:result.migrations?.map(item=>item.version)||undefined,
        emailAddress:result.provider==="gmail"?result.emailAddress||undefined:undefined,
      },
      tested_by:testedBy||"Subpar OS",
    });
  }catch{}
}
...
export async function runPreflight({scopes=["supabase","storage"],testedBy="Doug Talmadge"}={}){
  ...
  for(const scope of requested){
    ...
    tests.push(result);
    if(result.status!=="skipped")await storeConnectionTest(result,testedBy);
  }
  ...
  return {
    ok:failed.length===0,
    tests,
    summary:{requested:tests.length,passed:passed.length,failed:failed.length,skipped:tests.length-passed.length-failed.length},
    connectionTestGate:process.env.SUBPAR_CONNECTION_TESTS_ENABLED==="true",
    realDataApproved:process.env.SUBPAR_REAL_DATA_APPROVED==="true",
  };
}
