...
export const GO_LIVE_ENV_GROUPS = [
  {
    ...
    vars:["SUBPAR_SUPABASE_URL","NEXT_PUBLIC_SUBPAR_SUPABASE_URL","NEXT_PUBLIC_SUBPAR_SUPABASE_ANON_KEY","SUBPAR_SUPABASE_SERVICE_ROLE_KEY","SUBPAR_FILE_TICKET_SECRET"],
  },
  {
  ...
  },
  {
    ...
    vars:["SUBPAR_GOOGLE_CLIENT_ID","SUBPAR_GOOGLE_CLIENT_SECRET","SUBPAR_GOOGLE_REFRESH_TOKEN","SUBPAR_GMAIL_ACCOUNT","SUBPAR_GMAIL_PUBSUB_TOPIC"],
  },
  {
...
function envSet(name) {
  if (name === "SUBPAR_SUPABASE_URL") return Boolean(process.env.SUBPAR_SUPABASE_URL || process.env.NEXT_PUBLIC_SUBPAR_SUPABASE_URL);
...
export function getGoLiveReadiness() {
  ...
  const envGroups = GO_LIVE_ENV_GROUPS.map(group => ({
    ...group,
    values:group.vars.map(name => ({ name, configured:envSet(name) })),
  }));
...
export function planHistoricalImport({ provider, records, months, batchSize }) {
  ...
  return {
    provider:source,
    importType:source === "wix" ? "historical_orders" : "historical_threads",
    records:total || null,
    lookbackMonths,
    batchSize:size,
    batches,
    ...
    mutation:false,
    dedupe:source === "wix"
      ? ["external_source + external_order_id","customer email","existing external_links"]
      : ["Gmail thread id","Gmail message id","customer email","existing external_links"],
    conflictPolicy:[
      "Never auto-merge two existing customers with different emails",
      "Never overwrite an existing vehicle VIN/chassis conflict",
      "Never duplicate an order/thread already linked to a provider ID",
      "Ambiguous project matches go to manual review instead of guessing",
    ],
    phases:[
...
export function goLiveValidationSuite() {
  return [
...
export async function listImportBatches(limit = 10) {
  ...
  const { data, error } = await supabase
    .from("import_batches")
    .select("id,integration,import_type,status,mode,expected_count,scanned_count,created_count,matched_count,skipped_count,conflict_count,failed_count,created_at,updated_at")
