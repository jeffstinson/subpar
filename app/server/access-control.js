...
export const SESSION_COOKIE = "subpar_access_token";
...
export const demoPrincipals = {
  doug: { id:"demo_doug_talmadge", type:"internal", role:"owner", displayName:"Doug Talmadge", email:"doug@subpartuning.example" },
  tuner: { id:"demo_tuner", type:"internal", role:"tuner", displayName:"Subpar Tuner", email:"tuner@subpartuning.example" },
  staff: { id:"demo_staff", type:"internal", role:"staff", displayName:"Subpar Staff", email:"staff@subpartuning.example" },
  alex: { id:"demo_customer_alex", type:"customer", role:"customer", displayName:"Alex Rivera", email:"alex.rivera@gmail.com", customerId:"cus_alex-rivera", projectNumbers:["SP-1842"] },
};

export function can(principal, permission) {
  if (!principal) return false;
  if (principal.type === "system") return true;
  return (PERMISSIONS[permission] || []).includes(principal.role);
}
...
export function canAccessProject(principal, project) {
  ...
  return principal.type === "customer" && (principal.customerId === project.customerId || principal.projectNumbers?.includes(project.projectNumber));
}

export function canAccessFile(principal, file, project) {
  if (!principal || !file || !project || !canAccessProject(principal, project)) return false;
...
function bearerToken(request) {
  ...
  return parseCookie(request, SESSION_COOKIE);
}
...
export async function resolvePrincipalFromRequest(request, { demoFallback = null } = {}) {
  ...
  const { data:internalRows, error:internalError } = await supabase.from("internal_users").select("id,display_name,role,active").eq("auth_user_id", user.id).eq("active", true).limit(1);
  ...
  if (internalRows?.length) {
    ...
    return { id:user.id, type:"internal", role:row.role, displayName:row.display_name, email:user.email, membershipId:row.id };
  }

  const { data:portalRows, error:portalError } = await supabase.from("customer_portal_users").select("id,customer_id,active").eq("auth_user_id", user.id).eq("active", true).limit(1);
  if (portalError) throw new Error(`Unable to resolve customer identity: ${portalError.message}`);
  if (portalRows?.length) {
    const row = portalRows[0];
    return { id:user.id, type:"customer", role:"customer", displayName:user.user_metadata?.full_name || user.email, email:user.email, customerId:row.customer_id, portalMembershipId:row.id };
  }
  return null;
}
...
export function permissionMatrix() {
  ...
  return Object.entries(PERMISSIONS).map(([permission,allowed]) => ({ permission, roles:Object.fromEntries(roles.map(role => [role, allowed.includes(role)])) }));
}
